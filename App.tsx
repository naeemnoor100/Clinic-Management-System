    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 overflow-x-hidden relative transition-colors duration-300">
      ...
      <aside className="w-80 bg-slate-50 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 p-8 flex flex-col gap-10 sticky top-0 h-screen hidden md:flex print:hidden shrink-0 transition-colors">
        ...
        <div className="mt-auto pt-6 border-t border-slate-200 dark:border-slate-800">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-2">
            <div className="flex items-center justify-between"><p className="text-[8px] font-black uppercase text-slate-400 tracking-widest">Network Logic</p><div className={`w-2 h-2 rounded-full ${isSyncing ? 'bg-blue-500 animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.5)]' : syncStatus === 'success' ? 'bg-emerald-500' : syncStatus === 'error' ? 'bg-rose-500' : 'bg-slate-300'}`}></div></div><div className="flex items-center gap-2"><RefreshCw size={12} className={`text-slate-400 ${isSyncing ? 'animate-spin' : ''}`} /><span className="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-tighter">{isSyncing ? 'Syncing...' : syncStatus === 'error' ? 'Sync Error' : syncStatus === 'success' ? 'Cloud Connected' : 'Local Storage'}</span></div>{lastSyncedAt && (<p className="text-[8px] text-slate-400 font-bold uppercase tracking-tight">Updated: {lastSyncedAt}</p>)}
          </div>
        </div>
      </aside>

      <main className="flex-grow p-4 md:p-12 overflow-auto print:hidden pb-24 md:pb-12 transition-colors">
        <div className="max-w-6xl mx-auto w-full">
           {view === 'dashboard' && (
              <div className="space-y-6 md:space-y-10 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                  ...
                  <div onClick={() => setView('visits')} className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between cursor-pointer hover:scale-[1.02] transition-transform active:scale-95 group"><div><p className="text-slate-400 text-[10px] font-bold uppercase">Visits Today</p><p className="text-3xl md:text-4xl font-black text-slate-800 dark:text-white">{visits.filter(v => v.date === getCurrentIsoDate()).length}</p></div><Clock size={40} className="text-blue-200 dark:text-blue-800 group-hover:text-blue-500 transition-colors" /></div>
           ...
           {view === 'sync' && (
             <div className="space-y-8 animate-in fade-in">
                <div className="flex justify-between items-center"><h1 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white flex items-center gap-3"><Cloud className="text-blue-600" size={32}/> Secure API Cloud Network</h1><button onClick={() => setView('dashboard')} className="flex items-center gap-2 text-slate-400 dark:text-slate-500 hover:text-blue-600 font-black uppercase text-xs"><ArrowRight className="rotate-180" size={18} /> Dashboard</button></div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-xl space-y-10 transition-colors">
                    <div className="space-y-8">
                      <div className="flex items-center gap-4">
                        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-3xl"><Building2 size={32} /></div>
                        <div>
                      ...
                      <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-6">
                        <div className="flex items-center justify-between">
                           <div className="flex items-center gap-3">
                              ...
                              <div>
                                 ...
                                 <p className="text-[10px] text-slate-500 dark:text-slate-400">Backup automatically on every change</p>
                              </div>
                           </div>
                        ...
                        <div className="space-y-3 pt-2">
                           ...
                           <input 
                              ...
                              className="w-full p-4 rounded-xl border-2 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 font-bold text-xs outline-none focus:border-blue-500 transition-all text-slate-800 dark:text-white" 
                           />
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900 p-8 md:p-12 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-10 flex flex-col transition-colors">
                    <div className="space-y-8 flex-grow">
                      <div className="flex items-center gap-4">
                        <div className={`p-4 rounded-3xl ${isSyncing ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500 border border-slate-100 dark:border-slate-800'}`}>
                        ...
                        <div>
                          ...
                          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Manage manual backups and restoration tasks.</p>
                        </div>
                      </div>
                        ...
                        <div className="space-y-6">
                           <div className="grid grid-cols-2 gap-4">
                              <div className="p-6 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-3xl space-y-2">
                                 <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Last Verified Sync</p>
                                 <p className="text-sm font-black text-slate-800 dark:text-white">{lastSyncedAt || 'No History'}</p>
                              </div>
                              <div className="p-6 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-3xl space-y-2">
                                 ...
                                 <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-xs font-black text-slate-800 dark:text-white uppercase">{isAutoSyncEnabled ? 'Real-time On' : 'Manual Only'}</p>
                                 </div>
                              </div>
                           </div>
           ...
           {view === 'analytics' && (
             <div className="space-y-8 animate-in fade-in">
               <div className="flex justify-between items-center"><h1 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white">Clinic Analytics</h1><button onClick={() => setView('dashboard')} className="flex items-center gap-2 text-slate-400 dark:text-slate-500 hover:text-blue-600 font-black uppercase text-xs"><ArrowRight className="rotate-180" size={18} /> Dashboard</button></div>
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl md:rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6"><div className="flex items-center justify-between"><h3 className="font-black text-slate-800 dark:text-white flex items-center gap-2"><TrendingUp className="text-emerald-500" size={20}/> Revenue Trend (7 Days)</h3><span className="text-[10px] font-black text-slate-400 uppercase">Paid Total</span></div><div className="flex items-end justify-between h-48 gap-2 px-2">{analyticsData.revenueByDay.map((day, idx) => { const maxVal = Math.max(...analyticsData.revenueByDay.map(d => Number(d.total))) || 1; const height = (Number(day.total) / maxVal) * 100; return (<div key={idx} className="flex-1 flex flex-col items-center gap-2 group"><div className="w-full relative"><div style={{ height: `${Math.max(height, 5)}%` }} className="w-full bg-blue-500 rounded-t-lg group-hover:bg-blue-600 transition-all relative">{day.total > 0 && (<div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[9px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{CURRENCY} {day.total}</div>)}</div></div><span className="text-[9px] font-bold text-slate-400 uppercase">{day.date.split('-').slice(1).join('/')}</span></div>); })}</div></div><div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl md:rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6"><h3 className="font-black text-slate-800 dark:text-white flex items-center gap-2"><Pill className="text-indigo-500" size={20}/> Top Prescribed Medications</h3><div className="space-y-4">{analyticsData.topPrescribed.map(([name, count], idx) => { const maxCount = analyticsData.topPrescribed.length > 0 ? Number(analyticsData.topPrescribed[0][1]) : 1; const width = (Number(count) / maxCount) * 100; return (<div key={name} className="space-y-1"><div className="flex justify-between text-xs font-black text-slate-600 dark:text-slate-300"><span>{name}</span><span>{count} times</span></div><div className="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden"><div style={{ width: `${width}%` }} className={`h-full rounded-full transition-all duration-1000 bg-indigo-500`}/></div></div>); })}{analyticsData.topPrescribed.length === 0 && (<p className="text-center py-10 text-slate-300 font-bold uppercase text-xs">No records found</p>)}</div></div><div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl md:rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6"><h3 className="font-black text-slate-800 dark:text-white flex items-center gap-2"><Users className="text-blue-500" size={20}/> Patient Age Distribution</h3><div className="grid grid-cols-1 sm:grid-cols-3 gap-4">{Object.entries(analyticsData.ages).map(([group, count]) => { const total = patients.length || 1; const percent = Math.round((Number(count) / total) * 100); return (<div key={group} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center space-y-1"><p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{group}</p><p className="text-2xl font-black text-slate-800 dark:text-white">{count}</p><div className="text-[9px] font-bold text-blue-600 dark:text-blue-400">{percent}% of total</div></div>); })}</div></div><div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl md:rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6"><h3 className="font-black text-slate-800 dark:text-white flex items-center gap-2"><PieChart className="text-rose-500" size={20}/> Gender Demographics</h3><div className="flex items-center gap-6"><div className="flex-1 space-y-4">{Object.entries(analyticsData.genders).map(([gender, count]) => { const total = patients.length || 1; const width = (Number(count) / total) * 100; return (<div key={gender} className="space-y-1"><div className="flex justify-between text-[10px] font-black uppercase text-slate-500"><span>{gender}</span><span>{count}</span></div><div className="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden"><div style={{ width: `${width}%` }} className={`h-full rounded-full bg-${gender === 'Male' ? 'blue' : gender === 'Female' ? 'rose' : 'slate'}-500`}/></div></div>); })}</div><div className="hidden sm:block p-8 border-4 border-slate-50 dark:border-slate-800 rounded-full"><div className="text-center"><p className="text-3xl font-black text-slate-800 dark:text-white">{patients.length}</p><p className="text-[9px] font-bold text-slate-400 uppercase">Total Patients</p></div></div></div></div></div>
             </div>
           )}

           {view === 'patients' && (
              <div className="space-y-6 md:space-y-8 animate-in fade-in">
                 <div className="flex flex-col sm:flex-row justify-between items-center gap-4"><h1 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white w-full text-left">Patient Files</h1><div className="flex items-center gap-2 md:gap-4 w-full sm:w-auto"><div className="flex gap-1 shrink-0"><button onClick={exportPatientsCsv} className="p-2.5 md:p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-blue-600 rounded-xl md:rounded-2xl shadow-sm transition-all" title="Export to Excel (CSV)"><FileDown size={18} /></button><label className="p-2.5 md:p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-blue-600 rounded-xl md:rounded-2xl shadow-sm transition-all cursor-pointer" title="Import from Excel (CSV)"><FileUp size={18} /><input type="file" accept=".csv" className="hidden" onChange={handleImportPatientsCsv} /></label></div><div className="relative flex-grow sm:w-64 group"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={18} /><input type="text" placeholder="Search patients..." value={patientSearchTerm} onChange={(e) => setPatientSearchTerm(e.target.value)} className="w-full pl-10 pr-10 py-2.5 md:py-3 rounded-xl md:rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none font-bold text-sm transition-all shadow-sm" />{patientSearchTerm && <button onClick={() => setPatientSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors"><X size={16}/></button>}</div><button onClick={() => { setEditingPatient(null); setSelectedAllergies([]); setShowPatientForm(true); }} className="bg-blue-600 text-white px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black shadow-lg whitespace-nowrap text-sm">Register</button></div></div>
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">{filteredPatients.map(p => { const patientVisits = visits.filter(v => v.patientId === p.id).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()); const lastVisit = patientVisits[0]; return (<div key={p.id} onClick={() => { setSelectedPatientId(p.id); setView('patient-detail'); setDetailTab('history'); setHistorySearchTerm(''); }} className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-3xl md:rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm cursor-pointer hover:shadow-md transition-all group relative flex flex-col justify-between min-h-[140px]"><div><div className="flex items-center gap-4"><div className="w-10 h-10 md:w-12 md:h-12 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-400 font-black shrink-0">{p.name[0]}</div><div className="flex-grow min-w-0"><div className="flex items-center gap-2"><h3 className="font-black text-slate-800 dark:text-white truncate text-base">{p.name}</h3></div><p className="text-[11px] text-blue-600 dark:text-blue-400 font-bold leading-tight">{p.phone}</p>{p.allergies && (<p className="text-[10px] text-rose-500 font-bold truncate leading-tight mt-0.5">Allergy: {p.allergies}</p>)}<p className="text-[9px] text-slate-400 font-bold uppercase truncate mt-1">{p.patientCode} • {p.age}Y</p></div></div><div className="mt-4 flex flex-col gap-1.5">{lastVisit ? (<><div className="flex items-center gap-2 text-slate-500 dark:text-slate-400"><Calendar size={12} className="text-blue-400" /><p className="text-[10px] md:text-[11px] font-bold">Last Visit: <span className="text-slate-700 dark:text-slate-200">{formatDate(lastVisit.date)}</span></p></div>{lastVisit.vitals && Object.keys(lastVisit.vitals).length > 0 && (<div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1"><Activity size={10} className="text-emerald-500 shrink-0" /><p className="text-[9px] md:text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-tighter">{vitalDefinitions.map(vd => lastVisit.vitals![vd.id] ? `${vd.label}: ${lastVisit.vitals![vd.id]}${vd.unit}` : null).filter(Boolean).join(' • ')}</p></div>)}</>) : (<p className="text-[10px] md:text-[11px] text-slate-300 dark:text-slate-700 italic font-medium">No visits recorded yet</p>)}</div></div><div className="mt-4 pt-4 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between"><div className="flex gap-1"><button onClick={(e) => { e.stopPropagation(); launchEncounter(p); }} className="p-2 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors" title="Start Log Visit"><Stethoscope size={18} /></button><button onClick={(e) => { e.stopPropagation(); setEditingPatient(p); setSelectedAllergies(p.allergies ? p.allergies.split(', ').filter(Boolean) : []); setShowPatientForm(true); }} className="p-2 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"><Edit2 size={16} /></button><button onClick={(e) => { e.stopPropagation(); handleDeletePatient(p.id); }} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"><Trash2 size={16} /></button></div>{lastVisit && (<button onClick={(e) => { e.stopPropagation(); setQrVisit(lastVisit); }} className="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 p-2 md:p-2.5 rounded-lg md:rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-all shadow-sm flex items-center gap-2 group/qr"><QrCode size={18} /><span className="text-[9px] font-black uppercase tracking-tighter hidden sm:block">Visit QR</span></button>)}</div></div>); })}</div>
              </div>
           )}

           {view === 'patient-detail' && selectedPatientId && (
             <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right">{(() => { const p = patients.find(pat => pat.id === selectedPatientId); if (!p) return null; return (<div className="space-y-6 md:space-y-8"><div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><button onClick={() => setView('patients')} className="flex items-center gap-2 text-slate-400 dark:text-slate-500 hover:text-blue-600 font-black uppercase text-xs"><ArrowRight className="rotate-180" size={18} /> Back</button><div className="flex flex-wrap items-center gap-2"><button onClick={() => setQrPatient(p)} className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg hover:bg-indigo-700 transition-all active:scale-95"><QrCode size={16} /> Profile QR</button><button onClick={() => launchEncounter(p)} className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg hover:bg-emerald-700 transition-all active:scale-95"><Stethoscope size={16} /> Log Visit</button></div></div><div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-3xl md:rounded-[3rem] shadow-xl border border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-6 md:gap-10 transition-colors"><div className="flex-grow"><div className="flex justify-between items-start gap-4"><h2 className="text-2xl md:text-4xl font-black text-slate-800 dark:text-white leading-tight">{p.name}</h2><div className="flex gap-1 no-print shrink-0"><button onClick={() => setQrPatient(p)} className="p-2.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-400 hover:text-indigo-600 rounded-xl" title="Patient Profile QR"><QrCode size={18}/></button><button onClick={() => { setEditingPatient(p); setSelectedAllergies(p.allergies ? p.allergies.split(', ').filter(Boolean) : []); setShowPatientForm(true); }} className="p-2.5 bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-blue-600 rounded-xl" title="Edit Info"><Edit2 size={18}/></button><button onClick={() => handleDeletePatient(p.id)} className="p-2.5 bg-red-50 dark:bg-red-900/20 text-red-400 hover:text-red-600 rounded-xl" title="Delete"><Trash2 size={18}/></button></div></div><p className="text-slate-400 dark:text-slate-500 font-bold mt-2 uppercase text-[10px] md:text-xs">{p.patientCode} • {p.age}Y • {p.gender}</p><p className="text-slate-600 dark:text-slate-300 mt-4 font-medium text-sm">{p.phone} • {p.address}</p>{(p.allergies || p.chronicConditions || p.notes) && (<div className="mt-6 md:mt-8 flex flex-col sm:flex-row gap-3 md:gap-4 flex-wrap">{p.allergies && (<div className="bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/30 p-4 rounded-2xl flex items-start gap-3 flex-1 min-w-[200px]"><ShieldAlert className="text-rose-500 shrink-0" size={20} /><div><p className="text-[9px] md:text-[10px] font-black uppercase text-rose-400 tracking-widest">Allergies</p><p className="text-xs md:text-sm font-black text-rose-700 dark:text-rose-300">{p.allergies}</p></div></div>)}{p.chronicConditions && (<div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 p-4 rounded-2xl flex items-start gap-3 flex-1 min-w-[200px]"><Activity className="text-amber-500 shrink-0" size={20} /><div><p className="text-[9px] md:text-[10px] font-black uppercase text-amber-400 tracking-widest">Chronic Conditions</p><p className="text-xs md:text-sm font-black text-amber-700 dark:text-amber-300">{p.chronicConditions}</p></div></div>)}{p.notes && (<div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 p-4 rounded-2xl flex items-start gap-3 flex-1 min-w-[200px]"><StickyNote className="text-blue-500 shrink-0" size={20} /><div><p className="text-[9px] md:text-[10px] font-black uppercase text-blue-400 tracking-widest">Doctor's Notes</p><p className="text-xs md:text-sm font-black text-blue-700 dark:text-blue-300">{p.notes}</p></div></div>)}</div>)}</div><div className="bg-indigo-900 text-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] mt-6 lg:mt-0 w-full lg:max-w-xs space-y-4 shadow-xl shadow-indigo-100"><h3 className="text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-2"><Sparkles size={16}/> Clinical Analysis</h3><p className="text-xs md:text-sm italic opacity-80 leading-relaxed">"{aiSummary || "Analysis pending..."}"</p><button onClick={() => handleAiSummary(selectedPatientId!)} disabled={isSummarizing} className="w-full bg-white text-indigo-900 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black uppercase text-[9px] md:text-[10px] tracking-widest hover:bg-indigo-50 transition-all">{isSummarizing ? "Analyzing..." : "Analyze History"}</button></div></div><div className="space-y-6"><div className="flex gap-1 md:gap-2 bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-xl md:rounded-2xl w-full sm:w-fit no-print overflow-x-auto"><button onClick={() => setDetailTab('history')} className={`flex-1 sm:flex-none px-4 md:px-6 py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 whitespace-nowrap ${detailTab === 'history' ? 'bg-white dark:bg-slate-900 shadow text-blue-600 dark:text-blue-400' : 'text-slate-500 hover:text-slate-700'}`}><History size={14}/> History</button><button onClick={() => setDetailTab('prescriptions')} className={`flex-1 sm:flex-none px-4 md:px-6 py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 whitespace-nowrap ${detailTab === 'prescriptions' ? 'bg-white dark:bg-slate-900 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-500 hover:text-slate-700'}`}><FileText size={14}/> Meds</button></div>{detailTab === 'history' && (<div className="space-y-4 animate-in fade-in"><div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><h3 className="text-lg md:text-xl font-black text-slate-800 dark:text-white">Visit History</h3><div className="relative w-full sm:w-64 no-print group"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={16} /><input type="text" placeholder="Filter records..." value={historySearchTerm} onChange={(e) => setHistorySearchTerm(e.target.value)} className="w-full pl-10 pr-10 py-2.5 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none font-bold text-xs transition-all shadow-sm" />{historySearchTerm && (<button onClick={() => setHistorySearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors"><X size={14}/></button>)}</div></div><div className="space-y-3 md:space-y-4">{filteredHistory.map(v => (<div key={v.id} className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-2xl md:rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row justify-between sm:items-center gap-4 group transition-colors"><div className="flex-grow"><p className="text-[10px] text-slate-400 dark:text-slate-500 mb-1 font-bold">{formatDate(v.date)}</p><p className="font-bold text-slate-800 dark:text-white text-sm md:text-base">{v.diagnosis}</p><div className="flex flex-wrap gap-1">{v.symptoms ? v.symptoms.split(', ').map((s, i) => (<span key={i} className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-[9px] font-black rounded-full uppercase tracking-tighter">{s}</span>)) : <span className="text-slate-300 dark:text-slate-700 italic text-[10px]">No symptoms recorded</span>}</div><div className="mt-3 flex flex-wrap gap-2">{v.prescribedMeds.map((pm, i) => { const med = medications.find(m => m.id === pm.medicationId); const name = med ? med.brandName : pm.customName; return name ? (<span key={i} className="px-2 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg text-[10px] font-bold text-slate-600 dark:text-slate-400">{name} <span className="text-blue-500">×{pm.quantity}</span></span>) : null; })}</div></div><div className="flex gap-2 shrink-0 md:opacity-0 group-hover:opacity-100 transition-opacity justify-end no-print"><button onClick={() => handleWhatsAppShare(v, p)} className="bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 p-2.5 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/40 transition-all" title="Share WhatsApp"><MessageCircle size={16}/></button><button onClick={() => setQrVisit(v)} className="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 p-2.5 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-all" title="QR Code"><QrCode size={16}/></button><button onClick={() => triggerPrint(v)} className="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 p-2.5 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-all" title="Print"><Printer size={16}/></button></div></div>))}{filteredHistory.length === 0 && (<div className="py-12 md:py-20 text-center bg-white dark:bg-slate-900 rounded-3xl md:rounded-[2rem] border-2 border-dashed border-slate-100 dark:border-slate-800 px-4"><History size={40} className="mx-auto text-slate-200 dark:text-slate-800 mb-4"/><p className="text-slate-400 dark:text-slate-500 font-bold text-sm">{historySearchTerm ? `No results for "${historySearchTerm}"` : 'No clinical history found for this patient.'}</p></div>)}</div></div>)}{detailTab === 'prescriptions' && (<div className="space-y-4 animate-in slide-in-from-bottom"><h3 className="text-lg md:text-xl font-black text-slate-800 dark:text-white">Archive</h3><div className="bg-white dark:bg-slate-900 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden"><div className="overflow-x-auto custom-scrollbar"><table className="w-full text-left min-w-[500px]"><thead className="bg-slate-50 dark:bg-slate-800/50 text-[10px] font-black uppercase tracking-widest text-slate-400"><tr><th className="px-6 md:px-8 py-4 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => togglePrescSort('date')}>Date</th><th className="px-6 md:px-8 py-4 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => togglePrescSort('name')}>Medication</th><th className="px-6 md:px-8 py-4">Diagnosis</th><th className="px-6 md:px-8 py-4 text-right">Qty</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{patientPrescriptions.map((pr, idx) => (<tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"><td className="px-6 md:px-8 py-4 text-[11px] font-bold text-slate-400">{formatDate(pr.date)}</td><td className="px-6 md:px-8 py-4"><p className="font-black text-slate-800 dark:text-white text-xs md:text-sm">{pr.medName}</p><p className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase">{pr.strength}</p></td><td className="px-6 md:px-8 py-4"><p className="text-[11px] text-slate-600 dark:text-slate-400 italic line-clamp-1">{pr.diagnosis || '---'}</p></td><td className="px-6 md:px-8 py-4 text-right font-black text-indigo-600 dark:text-indigo-400 text-xs md:text-sm">{pr.quantity}</td></tr>))}</tbody></table></div></div></div>)}</div></div>); })()}</div>
           )}

           {view === 'visits' && (
             <div className="space-y-6 md:space-y-8 animate-in fade-in">
               <div className="flex flex-col sm:flex-row justify-between items-center gap-4"><h1 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white w-full text-left">Clinical Logs</h1><div className="flex items-center gap-2 md:gap-4 w-full sm:w-auto"><button onClick={exportVisitsCsv} className="p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-blue-600 rounded-xl md:rounded-2xl shadow-sm transition-all shrink-0" title="Export CSV"><FileDown size={18} /></button><div className="relative flex-grow sm:w-64 group"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={16} /><input type="text" placeholder="Search logs..." value={visitSearchTerm} onChange={(e) => setVisitSearchTerm(e.target.value)} className="w-full pl-10 pr-10 py-2.5 md:py-3 rounded-xl md:rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none font-bold text-sm transition-all shadow-sm" />{visitSearchTerm && <button onClick={() => setVisitSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors"><X size={16}/></button>}</div><button onClick={() => { setEditingVisit(null); setTempPrescribedMeds([]); setFormDiagnosis(''); setFormSelectedPatientId(null); setPatientFormSearch(''); setShowVisitForm(true); }} className="bg-blue-600 text-white px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black flex items-center gap-2 shadow-lg text-sm shrink-0"><PlusCircle size={18} /> <span className="hidden xs:inline">New Record</span></button></div></div>
               <div className="bg-white dark:bg-slate-900 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden"><div className="overflow-x-auto custom-scrollbar"><table className="w-full text-left min-w-[700px]"><thead className="bg-slate-50 dark:bg-slate-800/50 text-[10px] font-black uppercase tracking-widest text-slate-400"><tr><th className="px-6 md:px-8 py-5">Date</th><th className="px-6 md:px-8 py-5">Patient</th><th className="px-6 md:px-8 py-5">Symptoms</th><th className="px-6 md:px-8 py-5">Diagnosis</th><th className="px-6 md:px-8 py-5">Status</th><th className="px-6 md:px-8 py-5 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{filteredVisits.map(v => { const p = patients.find(pat => pat.id === v.patientId); return (<tr key={v.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group"><td className="px-6 md:px-8 py-5 font-bold text-slate-400 text-[11px] whitespace-nowrap">{formatDate(v.date)}</td><td className="px-6 md:px-8 py-5 whitespace-nowrap"><p className="font-black text-slate-800 dark:text-white text-xs md:text-sm">{p?.name}</p><p className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase tracking-tight">{p?.age}Y • {p?.phone}</p></td><td className="px-6 md:px-8 py-5"><div className="flex flex-wrap gap-1 max-w-[150px]">{v.symptoms ? v.symptoms.split(', ').map((s, i) => (<span key={i} className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-[9px] font-black rounded-full uppercase tracking-tighter">{s}</span>)) : <span className="text-slate-300 dark:text-slate-700 italic text-[10px]">None</span>}</div></td><td className="px-6 md:px-8 py-5 font-bold text-slate-700 dark:text-slate-300 text-xs md:text-sm">{v.diagnosis}</td><td className="px-6 md:px-8 py-5"><span className={`px-3 md:px-4 py-1 rounded-full text-[9px] font-black uppercase ${v.paymentStatus === 'Paid' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>{v.paymentStatus === 'Paid' ? 'Paid' : 'Unpaid'}</span></td><td className="px-6 md:px-8 py-5 text-right"><div className="flex items-center gap-1 md:gap-2 justify-end"><button onClick={() => triggerPrint(v)} className="p-2 text-slate-400 hover:text-blue-600" title="Print"><Printer size={16}/></button><button onClick={() => setQrVisit(v)} className="p-2 text-slate-400 hover:text-indigo-600" title="QR"><QrCode size={16}/></button><button onClick={() => { setEditingVisit(v); setTempPrescribedMeds(v.prescribedMeds || []); setFormDiagnosis(v.diagnosis || ''); setFormSelectedPatientId(v.patientId); setPatientFormSearch(patients.find(p => p.id === v.patientId)?.name || ''); setShowVisitForm(true); }} className="p-2 text-slate-400 hover:text-emerald-600" title="Edit"><Edit2 size={16}/></button><button onClick={() => handleDeleteVisit(v.id)} className="p-2 text-slate-400 hover:text-red-500" title="Delete Log"><Trash2 size={16}/></button></div></td></tr>); })}</tbody></table></div></div>
             </div>
           )}

           {view === 'pharmacy' && (
             <div className="flex flex-col gap-6 md:gap-8 animate-in fade-in h-full max-w-5xl mx-auto pb-10">
               <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 gap-4"><h1 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white flex items-center gap-3"><ShoppingCart className="text-blue-600" size={32} /> Pharmacy POS</h1><div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl w-full sm:w-auto"><button onClick={() => setPharmacyTab('pos')} className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${pharmacyTab === 'pos' ? 'bg-white dark:bg-slate-900 shadow text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}>New Billing</button><button onClick={() => setPharmacyTab('history')} className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${pharmacyTab === 'history' ? 'bg-white dark:bg-slate-900 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>Sales History</button></div></div>
               {pharmacyTab === 'pos' ? (
                 <div className="grid grid-cols-1 lg:grid-cols-1 gap-6"><div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-3xl md:rounded-[3rem] shadow-xl border-4 border-white dark:border-slate-800 space-y-8 transition-colors"><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2 relative"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2"><UserPlus size={14} className="text-blue-500" /> Customer Name / Patient</label><div className="relative"><input type="text" value={customerSearchTerm || walkinName} onFocus={() => setShowCustomerResults(true)} onChange={(e) => { setCustomerSearchTerm(e.target.value); setWalkinName(e.target.value); setShowCustomerResults(true); setSelectedPharmacyPatientId(null); }} placeholder="Search patient..." className="w-full p-4 pl-5 rounded-2xl border-2 border-slate-50 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white font-black text-sm outline-none focus:bg-white dark:focus:bg-slate-900 focus:border-blue-500/20 transition-all shadow-inner" />{showCustomerResults && customerSearchResults.length > 0 && (<div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-2xl rounded-2xl z-[100] overflow-hidden">{customerSearchResults.map(p => (<button key={p.id} type="button" onClick={() => { setWalkinName(p.name); setCustomerSearchTerm(p.name); setSelectedPharmacyPatientId(p.id); setShowCustomerResults(false); }} className="w-full text-left px-5 py-3 hover:bg-blue-50 dark:hover:bg-blue-900/30 border-b border-slate-50 dark:border-slate-800 font-black text-xs flex justify-between items-center group dark:text-slate-300"><span>{p.name}</span><span className="text-[8px] uppercase text-slate-300 group-hover:text-blue-400">{p.patientCode}</span></button>))}</div>)}</div></div><div className="space-y-2 relative"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2"><Search size={14} className="text-emerald-500" /> Add Medicine (Deep Search)</label><div className="relative"><input ref={posSearchRef} type="text" value={medSearchTerm} onChange={(e) => setMedSearchTerm(e.target.value)} placeholder="Formula, Brand, Type..." className="w-full p-4 pl-5 rounded-2xl border-2 border-slate-50 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white font-black text-sm outline-none focus:bg-white dark:focus:bg-slate-900 focus:border-emerald-500/20 transition-all shadow-inner" />{medSearchTerm && (<div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-2xl z-[100] max-h-64 overflow-y-auto custom-scrollbar overflow-x-hidden">{filteredMeds.length > 0 ? filteredMeds.map(med => { const isRisk = checkPharmacyMedAllergy(med.id); return (<button key={med.id} type="button" onClick={() => addToCart(med.id)} className={`w-full text-left px-5 py-4 border-b border-slate-50 dark:border-slate-800 last:border-none group flex flex-col gap-1 ${isRisk ? 'bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40' : 'hover:bg-emerald-50 dark:hover:bg-emerald-900/20'}`}><div className="flex justify-between items-start"><div className="flex items-center gap-2"><span className="font-black text-slate-800 dark:text-white text-sm">{med.brandName}</span>{isRisk && <span className="px-2 py-0.5 bg-red-600 text-white text-[8px] font-black rounded flex items-center gap-1 animate-pulse"><ShieldAlert size={10}/> Allergy Risk</span>}</div><span className="font-black text-emerald-600 dark:text-emerald-400 text-xs">{CURRENCY} {med.pricePerUnit}</span></div><span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight">{med.scientificName} • {med.strength} • {med.unit}</span><div className="flex justify-between items-center mt-1"><span className={`text-[8px] font-black uppercase ${med.stock <= med.reorderLevel ? 'text-rose-500' : 'text-slate-400 dark:text-slate-600'}`}>Stock: {med.stock} {med.unit}</span><Plus size={14} className="text-emerald-300 group-hover:text-emerald-600 transition-all" /></div></button>)}) : (<div className="p-8 text-center flex flex-col items-center gap-3"><PackageSearch size={32} className="text-slate-100 dark:text-slate-800" /><p className="text-[10px] font-black uppercase text-slate-300 dark:text-slate-700">No Match Found</p></div>)}</div>)}</div></div></div><div className="bg-slate-50/50 dark:bg-slate-950/50 rounded-[2.5rem] border-2 border-slate-100 dark:border-slate-800 overflow-hidden mt-8 transition-colors"><div className="overflow-x-auto custom-scrollbar"><table className="w-full text-left min-w-[600px]"><thead className="bg-white/80 dark:bg-slate-900/80 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400"><tr><th className="px-8 py-5">Sr.</th><th className="px-8 py-5">Medicine Information</th><th className="px-8 py-5 text-center">Quantity</th><th className="px-8 py-5 text-right">Sub-Total</th><th className="px-8 py-5 text-center w-20"></th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{cart.length === 0 ? (<tr><td colSpan={5} className="py-20 text-center"><div className="flex flex-col items-center gap-4 opacity-30"><ShoppingCart size={48} className="text-slate-200 dark:text-slate-800" /><p className="font-black text-xs uppercase text-slate-400 dark:text-slate-600 tracking-widest">Receipt is empty. Search and add items above.</p></div></td></tr>) : cart.map((item, idx) => { const med = medications.find(m => m.id === item.medicationId)!; const isRisk = checkPharmacyMedAllergy(item.medicationId); return (<tr key={item.medicationId} className={`transition-colors animate-in slide-in-from-left-4 ${isRisk ? 'bg-red-50/50 dark:bg-red-900/10 hover:bg-red-50 dark:hover:bg-red-900/20' : 'hover:bg-white/50 dark:hover:bg-slate-900/50'}`}><td className="px-8 py-6 font-black text-slate-300 dark:text-slate-700 text-xs">{idx + 1}</td><td className="px-8 py-6"><div className="flex flex-col"><span className="font-black text-slate-800 dark:text-white text-base flex items-center gap-2">{med.brandName} <span className="text-xs text-slate-400 dark:text-slate-500 font-bold">({med.scientificName})</span></span><span className="text-[10px] font-bold text-blue-500 dark:text-blue-400 uppercase mt-1 tracking-tighter">{med.strength} • {med.unit} • {CURRENCY} {med.pricePerUnit} each</span>{isRisk && (<span className="mt-2 text-red-600 dark:text-red-400 font-black text-[9px] uppercase flex items-center gap-1 animate-bounce"><ShieldAlert size={12}/> Allergy Alert: Matches Patient History</span>)}</div></td><td className="px-8 py-6"><div className="flex items-center justify-center gap-3 bg-white dark:bg-slate-900 w-fit mx-auto p-1.5 rounded-xl border-2 border-slate-100 dark:border-slate-800 shadow-sm"><button onClick={() => updateCartQty(item.medicationId, -1)} className="p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 transition-colors rounded-lg"><Minus size={14}/></button><input type="number" min="1" max={med.stock} value={item.quantity} onChange={(e) => { const val = parseInt(e.target.value) || 1; setCart(prev => prev.map(ci => ci.medicationId === item.medicationId ? { ...ci, quantity: Math.max(1, Math.min(val, med.stock)) } : ci)); }} className="font-black text-sm w-12 text-center outline-none bg-transparent dark:text-white" /><button onClick={() => updateCartQty(item.medicationId, 1)} className="p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 transition-colors rounded-lg"><Plus size={14}/></button></div></td><td className="px-8 py-6 text-right font-black text-slate-800 dark:text-white text-base">{CURRENCY} {(med.pricePerUnit * item.quantity).toLocaleString()}</td><td className="px-8 py-6 text-center"><button onClick={() => removeFromCart(item.medicationId)} className="p-3 text-slate-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-2xl transition-all"><Trash2 size={18}/></button></td></tr>); })}</tbody></table></div></div><div className="flex flex-col md:flex-row justify-between items-center gap-8 pt-6 border-t-4 border-dashed border-slate-100 dark:border-slate-800 transition-colors"><div className="text-center md:text-left"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Status Summary</p><p className="text-xs font-bold text-slate-600 dark:text-slate-400">Customer: <span className="text-blue-600 dark:text-blue-400">{walkinName}</span></p><p className="text-xs font-bold text-slate-600 dark:text-slate-400">Total Quantity: <span className="text-blue-600 dark:text-blue-400 font-black">{cartQuantityTotal}</span></p></div><div className="flex flex-col items-center md:items-end gap-6 w-full md:w-auto"><div className="text-center md:text-right"><span className="text-slate-400 dark:text-slate-500 font-black uppercase text-[10px] tracking-[0.25em] block mb-2">Grand Total</span><div className="flex items-center gap-4"><span className="text-4xl md:text-6xl font-black text-blue-600 dark:text-blue-400 tabular-nums">{CURRENCY} {cartTotal.toLocaleString()}</span></div></div><div className="flex gap-4 w-full md:w-auto"><button onClick={() => { if(window.confirm('Clear receipt?')) { setCart([]); setWalkinName('Walk-in Customer'); } }} disabled={cart.length === 0} className="px-8 py-5 rounded-3xl border-2 border-slate-100 dark:border-slate-800 text-slate-400 font-black uppercase tracking-widest text-[10px] hover:bg-rose-50 hover:text-rose-500 hover:border-rose-100 transition-all disabled:opacity-20">Clear</button><button onClick={completeSale} disabled={cart.length === 0} className="flex-grow md:flex-none px-12 py-5 bg-emerald-600 text-white rounded-[2rem] font-black uppercase tracking-[0.1em] shadow-2xl shadow-emerald-100 dark:shadow-none hover:bg-emerald-700 active:scale-95 transition-all text-base flex items-center justify-center gap-3"><CheckCircle2 size={24} /> Complete</button></div></div></div></div></div>) : (<div className="space-y-6 animate-in slide-in-from-right"><div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm transition-colors"><h2 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2"><History size={20} className="text-indigo-500" /> Sales Ledger</h2><div className="relative w-full sm:w-auto"><Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input type="date" value={pharmacyHistoryDate} onChange={(e) => setPharmacyHistoryDate(e.target.value)} className="w-full sm:w-auto pl-10 pr-4 py-2.5 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-white font-black text-xs outline-none focus:border-indigo-500 transition-all" />{pharmacyHistoryDate && <button onClick={() => setPharmacyHistoryDate('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500"><X size={14} /></button>}</div></div><div className="space-y-4">{groupedPharmacySales.length === 0 ? (<div className="py-20 text-center bg-white dark:bg-slate-900 rounded-3xl border-2 border-dashed border-slate-100 dark:border-slate-800"><Inbox size={48} className="mx-auto text-slate-100 dark:text-slate-800 mb-4" /><p className="font-black uppercase text-xs text-slate-300 dark:text-slate-700">No sales history found</p></div>) : groupedPharmacySales.map(group => (<div key={group.date} className="space-y-3"><div className="flex justify-between items-center px-4"><div className="flex items-center gap-3"><span className="bg-indigo-600 text-white px-4 py-1 rounded-full font-black text-[10px] shadow-sm">{formatDate(group.date)}</span><span className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">{group.sales.length} Transactions</span></div><div className="text-right"><p className="text-[8px] font-black text-slate-300 dark:text-slate-700 uppercase tracking-tighter">Daily Total Sale</p><p className="text-base font-black text-indigo-600 dark:text-indigo-400">{CURRENCY} {group.total.toLocaleString()}</p></div></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{group.sales.map(sale => (<div key={sale.id} onClick={() => setSelectedSaleForReceipt(sale)} className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all cursor-pointer group"><div className="flex justify-between items-start mb-3"><div className="flex-grow"><p className="font-black text-slate-800 dark:text-white text-sm group-hover:text-indigo-600 transition-colors">{sale.customerName}</p><p className="text-[9px] font-bold text-slate-400 uppercase">{sale.items.length} Items</p></div><p className="font-black text-emerald-600 dark:text-emerald-400 text-sm">{CURRENCY} {sale.totalAmount.toLocaleString()}</p></div><div className="flex justify-between items-center mt-4 pt-3 border-t border-slate-50 dark:border-slate-800"><div className="flex items-center gap-1.5 text-slate-300 dark:text-slate-700 text-[10px] font-bold"><Receipt size={12} /> ID: {sale.id.slice(0, 5).toUpperCase()}</div><button className="text-[9px] font-black uppercase text-indigo-400 group-hover:text-indigo-600 flex items-center gap-1">View Receipt <Eye size={12}/></button></div></div>))}</div></div>))}</div></div>)}
             </div>
           )}

           {view === 'billing' && (
              <div className="space-y-6 md:space-y-8 animate-in fade-in transition-colors"><div className="flex flex-col sm:flex-row justify-between items-center gap-4"><h1 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white w-full text-left">Billing</h1><div className="flex flex-wrap items-center gap-3 w-full sm:w-auto"><div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl"><button onClick={() => setBillingFilterStatus('All')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${billingFilterStatus === 'All' ? 'bg-white dark:bg-slate-900 shadow text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}>All</button><button onClick={() => setBillingFilterStatus('Pending')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${billingFilterStatus === 'Pending' ? 'bg-white dark:bg-slate-900 shadow text-rose-600 dark:text-rose-400' : 'text-slate-400 dark:text-slate-500'}`}>Only Unpaid</button></div><div className="relative"><input type="date" value={billingDate} onChange={(e) => setBillingDate(e.target.value)} className="w-full sm:w-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 font-black text-xs outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-800 dark:text-white"/>{billingDate && (<button onClick={() => setBillingDate('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400"><X size={14} /></button>)}</div></div></div><div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><div className="bg-indigo-600 p-6 rounded-3xl text-white shadow-lg"><p className="text-[9px] font-bold uppercase opacity-80">Total Revenue</p><p className="text-2xl md:text-3xl font-black mt-1">{CURRENCY} {billingStats.total.toLocaleString()}</p></div><div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm"><p className="text-[9px] font-bold uppercase text-slate-400 dark:text-slate-500">Consultations</p><p className="text-xl md:text-2xl font-black text-blue-600 dark:text-blue-400 mt-1">{CURRENCY} {billingStats.consultations.toLocaleString()}</p></div><div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm"><p className="text-[9px] font-bold uppercase text-slate-400 dark:text-slate-500">Pharmacy</p><p className="text-xl md:text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1">{CURRENCY} {billingStats.pharmacy.toLocaleString()}</p></div></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8"><div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col h-full min-h-[300px]"><h2 className="text-lg font-black mb-4 flex items-center gap-2 dark:text-white"><Stethoscope size={18} className="text-blue-500" /> Consultations</h2><div className="space-y-4 overflow-y-auto custom-scrollbar flex-grow pr-1">{filteredBillingConsultations.map(v => (<div key={v.id} className="flex justify-between items-center border-b border-slate-50 dark:border-slate-800 pb-3"><div><p className="font-bold text-slate-800 dark:text-white text-sm">{patients.find(p => p.id === v.patientId)?.name || 'Unknown'}</p><p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase">{formatDate(v.date)}</p></div><div className="text-right"><p className="font-black text-blue-600 dark:text-blue-400 text-sm">{CURRENCY} {v.feeAmount}</p><span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full ${v.paymentStatus === 'Paid' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-500' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-500'}`}>{v.paymentStatus === 'Paid' ? 'Paid' : 'Unpaid'}</span></div></div>))}</div></div><div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col h-full min-h-[300px]"><h2 className="text-lg font-black mb-4 flex items-center gap-2 dark:text-white"><ShoppingCart size={18} className="text-indigo-500" /> Pharmacy</h2><div className="space-y-4 overflow-y-auto custom-scrollbar flex-grow pr-1">{filteredBillingPharmacy.map(s => (<div key={s.id} className="flex justify-between items-center border-b border-slate-50 dark:border-slate-800 pb-3"><div><p className="font-bold text-slate-800 dark:text-white text-sm">{s.customerName}</p><p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase">{formatDate(s.date)}</p></div><div className="text-right"><p className="font-black text-indigo-600 dark:text-indigo-400 text-sm">{CURRENCY} {s.totalAmount}</p></div></div>))}</div></div></div></div>
           )}

           {view === 'settings' && (
             <div className="space-y-6 md:space-y-8 animate-in fade-in transition-colors">
                <div className="flex flex-col gap-4">
                  <div className="flex justify-between items-center"><h1 className="text-2xl font-black text-slate-800 dark:text-white">Settings</h1>{settingsTab === 'meds' && (<div className="flex gap-1"><button onClick={exportMedsCsv} className="p-2.5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-blue-600 rounded-xl shadow-sm transition-all" title="Export Meds (CSV)"><FileDown size={18} /></button><label className="p-2.5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-blue-600 rounded-xl shadow-sm transition-all cursor-pointer" title="Import Meds (CSV)"><FileUp size={18} /><input type="file" accept=".csv" className="hidden" onChange={handleImportMedsCsv} /></label></div>)}</div>
                  <div className="flex gap-2 bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-xl w-full overflow-x-auto no-scrollbar scroll-smooth">{[{ id: 'appearance', label: 'Font System', icon: <Type size={12} className="text-blue-500"/> },{ id: 'low_stock', label: 'Critical Stock', icon: <ShieldAlert size={12} className="text-rose-500"/> },{ id: 'vitals', label: 'Vitals', icon: <Activity size={12} className="dark:text-slate-400"/> },{ id: 'symptoms', label: 'Symptoms', icon: <Droplets size={12} className="dark:text-slate-400"/> },{ id: 'templates', label: 'Templates', icon: <LayoutTemplate size={12} className="dark:text-slate-400"/> },{ id: 'meds', label: 'Medicines', icon: <Pill size={12} className="dark:text-slate-400"/> },{ id: 'scientific', label: 'Scientific', icon: <FlaskConical size={12} className="dark:text-slate-400"/> },{ id: 'companies', label: 'Companies', icon: <Building2 size={12} className="dark:text-slate-400"/> },{ id: 'med_categories', label: 'Cats', icon: <Layers size={12} className="dark:text-slate-400"/> },{ id: 'med_types', label: 'Types', icon: <Tags size={12} className="dark:text-slate-400"/> }].map(tab => (<button key={tab.id} onClick={() => { setSettingsTab(tab.id as any); setSettingsSearchTerm(''); }} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 whitespace-nowrap relative ${settingsTab === tab.id ? 'bg-white dark:bg-slate-900 shadow text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>{tab.icon} {tab.label}{tab.id === 'low_stock' && stats.lowStockCount > 0 && (<span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-white dark:border-slate-800"></span>)}</button>))}</div>
                  {settingsTab !== 'appearance' && (
                    <div className="flex flex-col sm:flex-row gap-4">
                      <div className="relative flex-grow group"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={16} /><input type="text" placeholder={`Deep Search in ${settingsTab}...`} value={settingsSearchTerm} onChange={(e) => setSettingsSearchTerm(e.target.value)} className="w-full pl-10 pr-10 py-2.5 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none font-bold transition-all text-sm shadow-sm" />{settingsSearchTerm && <button onClick={() => setSettingsSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors"><X size={14}/></button>}</div>
                    </div>
                  )}
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 animate-in slide-in-from-bottom-4">
                  {settingsTab === 'appearance' && (<div className="col-span-full space-y-10">
                    <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-10 transition-colors">
                      <div className="flex items-center gap-4"><div className="p-4 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-3xl"><Type size={32} /></div><div><h2 className="text-2xl font-black text-slate-800 dark:text-white">Application Text Scale</h2><p className="text-slate-400 dark:text-slate-500 text-sm font-medium">Customize the global font size for better readability.</p></div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{[{ label: 'Small', size: 14 },{ label: 'Normal', size: 16 },{ label: 'Large', size: 18 },{ label: 'X-Large', size: 20 }].map((opt) => (<button key={opt.size} onClick={() => setFontSize(opt.size)} className={`p-8 rounded-[2rem] border-4 transition-all flex flex-col items-center gap-3 active:scale-95 ${fontSize === opt.size ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 shadow-xl' : 'border-slate-50 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 text-slate-400'}`}><span className="font-black" style={{ fontSize: `${opt.size}px` }}>Aa</span><span className="text-[10px] font-black uppercase tracking-widest">{opt.label}</span></button>))}</div>
                    </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 px-2 py-2 flex items-center justify-between z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] print:hidden transition-colors"><button onClick={() => setView('dashboard')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${view === 'dashboard' ? 'text-blue-600 scale-110' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}><LayoutDashboard size={18} /><span className="text-[8px] font-black uppercase">Home</span></button><button onClick={() => setView('patients')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${view === 'patients' || view === 'patient-detail' ? 'text-blue-600 scale-110' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}><Users size={18} /><span className="text-[8px] font-black uppercase">Files</span></button><button onClick={() => setView('visits')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${view === 'visits' ? 'text-blue-600 scale-110' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}><ClipboardList size={18} /><span className="text-[8px] font-black uppercase">Logs</span></button><button onClick={() => setView('pharmacy')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${view === 'pharmacy' ? 'text-blue-600 scale-110' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}><Pill size={18} /><span className="text-[8px] font-black uppercase">Med</span></button><button onClick={() => setView('sync')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${view === 'sync' ? 'text-blue-600 scale-110' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}><Cloud size={18} /><span className="text-[8px] font-black uppercase">Sync</span></button><button onClick={() => setView('settings')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${view === 'settings' ? 'text-blue-600 scale-110' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}><Settings size={18} /><span className="text-[8px] font-black uppercase">Setup</span></button></nav>

      {selectedSaleForReceipt && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[700] flex items-center justify-center p-0 sm:p-4 animate-in zoom-in" onClick={() => setSelectedSaleForReceipt(null)}><div className="bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-w-2xl sm:rounded-[3rem] shadow-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}><div className="p-6 md:p-8 bg-indigo-600 text-white flex justify-between items-center shrink-0"><div className="flex items-center gap-3"><Receipt size={24} /><div><h2 className="text-xl font-black tracking-tight">Sales Receipt</h2><p className="text-[10px] font-bold opacity-70 uppercase tracking-widest">ID: {selectedSaleForReceipt.id.toUpperCase()}</p></div></div><button onClick={() => setSelectedSaleForReceipt(null)} className="text-3xl hover:opacity-70 transition-opacity">&times;</button></div><div className="p-6 md:p-10 space-y-8 flex-grow overflow-y-auto custom-scrollbar"><div className="flex justify-between items-end border-b-2 border-slate-50 dark:border-slate-800 pb-6"><div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Customer Details</p><p className="text-lg font-black text-slate-800 dark:text-white">{selectedSaleForReceipt.customerName}</p><p className="text-xs font-bold text-slate-500 dark:text-slate-400">{formatDate(selectedSaleForReceipt.date)}</p></div><div className="text-right"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Payment Status</p><span className="px-4 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full font-black text-[10px] uppercase">Paid</span></div></div><div className="space-y-4"><table className="w-full text-left"><thead className="text-[9px] font-black uppercase text-slate-400 tracking-widest border-b border-slate-100 dark:border-slate-800"><tr><th className="py-3">Medicine (Formula - Strength)</th><th className="py-3 text-center">Qty</th><th className="py-3 text-right">Subtotal</th></tr></thead><tbody className="divide-y divide-slate-50 dark:divide-slate-800">{selectedSaleForReceipt.items.map((item, idx) => { const med = medications.find(m => m.id === item.medicationId); return (<tr key={idx} className="text-xs"><td className="py-4"><p className="font-black text-slate-800 dark:text-white">{med?.brandName || 'Unknown Med'}</p><p className="text-[9px] font-bold text-slate-400 italic">({med?.scientificName}) - {med?.strength}</p></td><td className="py-4 text-center font-black text-slate-600 dark:text-slate-400">{item.quantity}</td><td className="py-4 text-right font-black text-slate-800 dark:text-white">{CURRENCY} {(item.quantity * item.priceAtTime).toLocaleString()}</td></tr>); })}</tbody></table></div><div className="pt-6 border-t-4 border-dashed border-slate-100 dark:border-slate-800 flex flex-col items-end gap-2"><div className="flex flex-col items-end"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Quantity</span><p className="text-xl font-black text-slate-600 dark:text-slate-400">{selectedSaleForReceipt.items.reduce((sum, item) => sum + item.quantity, 0)} Items</p></div><div className="flex flex-col items-end mt-2"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Grand Total</span><p className="text-4xl font-black text-indigo-600 dark:text-indigo-400 tabular-nums">{CURRENCY} {selectedSaleForReceipt.totalAmount.toLocaleString()}</p></div></div></div><div className="p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700 flex gap-3"><button onClick={() => window.print()} className="flex-1 py-4 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"><Printer size={16}/> Print Copy</button><button onClick={() => setSelectedSaleForReceipt(null)} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-indigo-100 active:scale-95 transition-all">Close</button></div></div></div>)}
      {qrVisit && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[600] flex items-center justify-center p-0 sm:p-4" onClick={() => setQrVisit(null)}><div className="bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-w-md sm:rounded-[3.5rem] p-8 md:p-12 shadow-2xl flex flex-col items-center text-center gap-6 animate-in zoom-in" onClick={e => e.stopPropagation()}><div className="w-full flex justify-end sm:hidden"><button onClick={() => setQrVisit(null)} className="text-slate-400 text-3xl">&times;</button></div><div className="bg-indigo-50 dark:bg-indigo-900/30 p-6 rounded-3xl text-indigo-600 dark:text-indigo-400 shadow-inner"><QrCode size={48} /></div><div><h2 className="text-xl md:text-2xl font-black text-slate-800 dark:text-white">Scan Visit QR</h2></div><div className="p-3 md:p-4 bg-white rounded-3xl shadow-lg"><QRCodeCanvas value={getVisitQrData(qrVisit)} size={200} level="M" /></div><div className="text-left w-full text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl whitespace-pre-wrap font-mono">{getVisitQrData(qrVisit)}</div><button onClick={() => setQrVisit(null)} className="w-full py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">Dismiss</button></div></div>)}
      {qrPatient && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[600] flex items-center justify-center p-0 sm:p-4" onClick={() => setQrPatient(null)}><div className="bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-w-md sm:rounded-[3.5rem] p-8 md:p-12 shadow-2xl flex flex-col items-center text-center gap-6 animate-in zoom-in" onClick={e => e.stopPropagation()}><div className="w-full flex justify-end sm:hidden"><button onClick={() => setQrPatient(null)} className="text-slate-400 text-3xl">&times;</button></div><div className="bg-blue-50 dark:bg-blue-900/30 p-6 rounded-3xl text-blue-600 dark:text-blue-400 shadow-inner"><User size={48} /></div><div><h2 className="text-xl md:text-2xl font-black text-slate-800 dark:text-white">Patient Profile QR</h2></div><div className="p-3 md:p-4 bg-white rounded-3xl shadow-lg"><QRCodeCanvas value={getPatientQrData(qrPatient)} size={240} level="M" /></div><div className="text-left w-full max-h-40 overflow-y-auto text-[10px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl whitespace-pre-wrap font-mono custom-scrollbar">{getPatientQrData(qrPatient)}</div><button onClick={() => setQrPatient(null)} className="w-full py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">Dismiss</button></div></div>)}

      {showTemplateForm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[500] flex items-center justify-center p-0 sm:p-4">
           <div className="bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-w-2xl sm:rounded-[3rem] shadow-2xl flex flex-col animate-in slide-in-from-bottom sm:zoom-in transition-colors">
             ...
             <form onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); const name = f.get('tplName') as string; const minAge = f.get('minAge') ? parseInt(f.get('minAge') as string) : undefined; const maxAge = f.get('maxAge') ? parseInt(f.get('maxAge') as string) : undefined; const finalPrescribedMeds = tempPrescribedMeds.map(({searchTerm, ...rest}) => { if (!rest.medicationId && searchTerm) return { ...rest, customName: searchTerm }; return rest; }).filter(pm => pm.medicationId !== '' || pm.customName !== ''); const d: PrescriptionTemplate = { id: editingTemplate ? editingTemplate.id : Math.random().toString(36).substr(2, 9), name, diagnosis: formDiagnosis, prescribedMeds: finalPrescribedMeds, minAge, maxAge }; if (editingTemplate) setPrescriptionTemplates(prev => prev.map(i => i.id === editingTemplate.id ? d : i)); else setPrescriptionTemplates(prev => [...prev, d]); setShowTemplateForm(false); setEditingTemplate(null); setTempPrescribedMeds([]); setFormDiagnosis(''); }} className="p-6 md:p-8 space-y-6 flex-grow overflow-y-auto custom-scrollbar">
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">Template Name</label><input required name="tplName" defaultValue={editingTemplate?.name} placeholder="e.g. Cough & Flu" className="w-full p-4 rounded-xl md:rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" /></div>
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">Min Age (Years)</label><input type="number" name="minAge" defaultValue={editingTemplate?.minAge} placeholder="No min" className="w-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" /></div>
                   <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">Max Age (Years)</label><input type="number" name="maxAge" defaultValue={editingTemplate?.maxAge} placeholder="No max" className="w-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" /></div>
                </div>
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">Diagnosis</label><input name="diagnosis" value={formDiagnosis} onChange={e => setFormDiagnosis(e.target.value)} placeholder="Default diagnosis" className="w-full p-4 rounded-xl md:rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" /></div>
                <div className="space-y-4"><div className="flex justify-between items-center"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Medications</label><button type="button" onClick={() => setTempPrescribedMeds([...tempPrescribedMeds, { medicationId: '', dosage: '', frequency: '', duration: '', searchTerm: '', quantity: 0 }])} className="text-blue-600 dark:text-blue-400 font-black text-[10px] uppercase">+ Add</button></div><div className="space-y-3">{tempPrescribedMeds.map((pm, idx) => { const selectedMed = medications.find(m => m.id === pm.medicationId); const isSearching = activeMedSearchIndex === idx; const filteredMedsList = medications.filter(m => m.brandName.toLowerCase().includes((pm.searchTerm || '').toLowerCase()) || m.scientificName.toLowerCase().includes((pm.searchTerm || '').toLowerCase())); return (<div key={idx} className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border-2 border-slate-100 dark:border-slate-800 flex flex-col gap-3"><div className="flex items-center gap-3"><div className="flex-grow relative group"><input type="text" placeholder="Search inventory..." value={pm.searchTerm !== undefined ? pm.searchTerm : (selectedMed?.brandName || pm.customName || '')} onFocus={() => setActiveMedSearchIndex(idx)} onChange={(e) => { const newList = [...tempPrescribedMeds]; newList[idx].searchTerm = e.target.value; if (newList[idx].medicationId) newList[idx].medicationId = ''; setTempPrescribedMeds(newList); }} className="w-full font-black text-sm outline-none bg-transparent dark:text-white" />{isSearching && (<div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-2xl z-[400] max-h-40 overflow-y-auto">{filteredMedsList.length > 0 ? (filteredMedsList.map(m => (<button key={m.id} type="button" onClick={() => { const newList = [...tempPrescribedMeds]; newList[idx].medicationId = m.id; newList[idx].customName = undefined; newList[idx].searchTerm = undefined; setTempPrescribedMeds(newList); setActiveMedSearchIndex(null); }} className="w-full text-left px-4 py-2.5 hover:bg-blue-50 dark:hover:bg-blue-900/30 border-b border-slate-50 dark:border-slate-800 font-black text-[10px] truncate dark:text-white">{m.brandName} <span className="text-[8px] text-slate-400 font-bold uppercase ml-1">({m.scientificName})</span></button>))) : pm.searchTerm?.trim() ? (<button type="button" onClick={() => { const newList = [...tempPrescribedMeds]; newList[idx].customName = pm.searchTerm; newList[idx].medicationId = ''; newList[idx].searchTerm = undefined; setTempPrescribedMeds(newList); setActiveMedSearchIndex(null); }} className="w-full text-left px-4 py-3 bg-blue-50/50 dark:bg-blue-900/20 hover:bg-blue-50 dark:hover:bg-blue-900/40 font-black text-[9px] uppercase text-blue-600 dark:text-blue-400 flex items-center gap-2"><Plus size={12} /> Add "{pm.searchTerm}"</button>) : (<div className="p-3 text-slate-300 text-[9px] uppercase text-center">No Match</div>)}</div>)}</div><button type="button" onClick={() => setTempPrescribedMeds(tempPrescribedMeds.filter((_, i) => i !== idx))} className="text-slate-300 dark:text-slate-700"><Trash2 size={16}/></button></div><div className="grid grid-cols-2 gap-2"><input placeholder="Dosage" value={pm.dosage} onChange={e => { const nl = [...tempPrescribedMeds]; nl[idx].dosage = e.target.value; setTempPrescribedMeds(nl); }} className="p-2 bg-white dark:bg-slate-900 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-800 dark:text-white" /><input type="number" placeholder="Qty" value={pm.quantity || ''} onChange={e => { const nl = [...tempPrescribedMeds]; nl[idx].quantity = parseInt(e.target.value) || 0; setTempPrescribedMeds(nl); }} className="p-2 bg-white dark:bg-slate-900 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-800 dark:text-white" /></div></div>); })}</div></div><button type="submit" className="w-full bg-blue-600 text-white py-4 md:py-5 rounded-2xl md:rounded-3xl font-black uppercase tracking-widest shadow-xl text-sm mt-4">Save Template</button>
             </form>
           </div>
        </div>
      )}
      ...
      {showPatientForm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[500] flex items-center justify-center p-0 sm:p-4">
           <div className="bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-lg sm:rounded-[3rem] shadow-2xl flex flex-col animate-in slide-in-from-bottom sm:zoom-in transition-colors">
             ...
             <form onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); const allergiesStr = selectedAllergies.join(', '); const d = { name: f.get('name') as string, age: parseInt(f.get('age') as string), gender: f.get('gender') as any, phone: f.get('phone') as string, address: f.get('address') as string, allergies: allergiesStr, chronicConditions: f.get('chronicConditions') as string, notes: f.get('notes') as string }; if (editingPatient) setPatients(prev => prev.map(i => i.id === editingPatient.id ? { ...i, ...d } : i)); else { const nextNum = patientCounter + 1; const code = `P-${nextNum.toString().padStart(4, '0')}`; setPatientCounter(nextNum); setPatients(prev => [...prev, { id: Math.random().toString(36).substr(2, 9), patientCode: code, ...d }]); } setShowPatientForm(false); setEditingPatient(null); setAllergySearchTerm(''); setSelectedAllergies([]); }} className="p-6 md:p-8 space-y-5 flex-grow overflow-y-auto custom-scrollbar">
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Full Name</label><input required name="name" defaultValue={editingPatient?.name} className="w-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" /></div>
                <div className="grid grid-cols-2 gap-4"><div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Age</label><input required type="number" name="age" defaultValue={editingPatient?.age} className="w-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" /></div><div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Gender</label><select name="gender" defaultValue={editingPatient?.gender} className="w-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white"><option value="Male">Male</option><option value="Female">Female</option><option value="Other">Other</option></select></div></div>
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Phone</label><input required name="phone" defaultValue={editingPatient?.phone} className="w-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" /></div>
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Address</label><textarea required name="address" defaultValue={editingPatient?.address} className="w-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-black text-sm dark:text-white" rows={2} /></div>
      ...
      {showVisitForm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[500] flex items-center justify-center p-0 sm:p-4 animate-in slide-in-from-bottom sm:zoom-in">
          <div className="bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-w-5xl sm:rounded-[3.5rem] shadow-2xl flex flex-col overflow-hidden transition-colors">
             ...
             <form onSubmit={handleVisitSubmit} className="p-6 md:p-10 space-y-8 md:space-y-10 bg-slate-50/30 dark:bg-slate-950/30 flex-grow overflow-y-auto custom-scrollbar">
                {/* Form Content */}
      ...
      <div className="hidden print:block print-only bg-white text-black font-sans leading-tight z-[1000] print-container">{printingVisit ? (() => { const p = patients.find(pat => pat.id === printingVisit.patientId); return (<div className="w-full p-4 space-y-4 text-[13px]"><div className="text-center border-b-2 border-black pb-4 mb-4"><h1 className="text-2xl font-black">SmartClinic</h1><p className="font-bold uppercase tracking-widest">Medical Hub</p></div><div className="space-y-3"><div className="flex"><span className="font-bold w-24">Name:</span> <span>{p?.name || '---'}</span></div></div></div>); })() : null}</div>
    </div>
  );
};
